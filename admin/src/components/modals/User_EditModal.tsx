...
export default function UserEditModal({ isOpen, onClose, user, onUpdate }: UserEditModalProps) {
  ...
  useEffect(() => {
    if (user) {
      ...
      if (user.image) {
        setPreviewUrl(`http://192.168.1.24:5000${user.image}`);
      }
    }
  }, [user]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Append permissions as a string array
      ...
      const response = await fetch(`http://192.168.1.24:5000/api/user/${user._id}`, {
        method: 'PUT',
        body: formDataToSend,
      });
